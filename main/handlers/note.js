...
class NoteHandlers {
    async create(payload) {
        try {
            const results = await noteServices.create(payload);
    ...
    async get(page = 1, limit = 10, filters = null, search = false) {
        try {
            ...
            return {
                success: true,
                ...results,
                data: results.data.map((note) => note.toJSON()),
            };
        } catch (err) {
            return errorHandler(err);
        }
    ...
    async update(id, payload) {
        try {
            const results = await noteServices.update(id, payload);
...
export default new NoteHandlers();
